<template>
  <el-dialog :visible="showDialog" title="新增员工" width="800px" @close="btnCancel">
    <el-form ref="addEmployee" :model="formData" :rules="rules" label-width="120px">
      ...
      <el-form-item label="部门" prop="departmentName">
        ...
        <el-tree v-if="showTree" :data="treeData" :props="{label:'name'}" @node-click="select" />
      </el-form-item>
...
export default {
    ...
    return {
      ...
      rules: {
        username: [{ required: true, message: '用户姓名不能为空', trigger: 'blur' }, {
          min: 1, max: 50, message: '用户姓名为1-4位'
        }],
  ...
  methods: {
    ...
    async   getDepartments() {
      ...
      this.treeData = transFormTreeStructure(depts, '')
      this.showTree = true
    },
    select(node) {
      this.showTree = false
